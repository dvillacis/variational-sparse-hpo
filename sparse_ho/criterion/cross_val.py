...
class CrossVal(BaseCriterion):
    ...
    def save_state(self):
        ...
        return {
            "dict_crits": {
                i: self.dict_crits[i].save_state() for i in range(self.n_splits)
            }
        }

    def restore_state(self, state):
        if state is None or self.dict_crits is None:
            return
        for i in range(self.n_splits):
            self.dict_crits[i].restore_state(state["dict_crits"][i])
