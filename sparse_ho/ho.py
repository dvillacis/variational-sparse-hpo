...
def _attach_state_api(get_val_grad, criterion):
    get_val_grad.save_state = criterion.save_state
    get_val_grad.restore_state = criterion.restore_state
    return get_val_grad
...
def grad_search(
    ...
    fallback_algo: instance of BaseAlgo, optional (default=None)
        alternative hypergradient algorithm exposed to the optimizer as a
        fallback direction source. Only optimizers with an accept/reject
        test use it (e.g. TrustRegion retries a rejected step along the
        fallback direction at the same radius before contracting).
    ...
    def _get_val_grad(log_alpha, tol, monitor):
        return criterion.get_val_grad(
            ...
            monitor=monitor)
    _attach_state_api(_get_val_grad, criterion)

    if fallback_algo is not None:
        def _get_val_grad_fallback(log_alpha, tol, monitor):
            return criterion.get_val_grad(
                model, X, y, log_alpha, fallback_algo.compute_beta_grad,
                tol=tol, monitor=monitor)
        _get_val_grad.fallback = _get_val_grad_fallback
    ...
    try:
        ...
        if exc.name == "pkg_resources":
            raise ModuleNotFoundError(
                "hyperopt requires setuptools/pkg_resources in this environment. "
                "Install `setuptools` to use `hyperopt_wrapper`."
            ) from exc
        raise
